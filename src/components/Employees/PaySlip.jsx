...
import { Search, Plus, Edit, Trash2, Save, X, Upload, Eye, Calculator, ArrowLeft } from 'lucide-react';
     ...
     const  PaySlip = () => {
                   ...
                   const [formData, setFormData] = useState({
                        ...
                        const grossEarnings = formData.basicSalary + formData.hra + formData.conveyanceAllowance + 
                                      formData.medicalAllowance + formData.specialAllowance + formData.bonus + 
                                      formData.overtimePay + formData.otherEarnings;
  
                        const totalDeductions = formData.pf + formData.esi + formData.professionalTax + 
                                                formData.incomeTax + formData.loanRecovery + formData.otherDeductions;
                        
                        const netSalary = grossEarnings - totalDeductions;

                        const handleInputChange = (field, value) => {
                            const numericValue = ['basicSalary', 'hra', 'conveyanceAllowance', 'medicalAllowance', 'specialAllowance', 
                                                'bonus', 'overtimePay', 'otherEarnings', 'pf', 'esi', 'professionalTax', 
                                                'incomeTax', 'loanRecovery', 'otherDeductions'].includes(field) 
                ...
                const resetForm = () => {
                        setFormData({
                        employeeId: '', employeeName: '', department: '', designation: '', payPeriod: '', paymentDate: '',
                        basicSalary: 0, hra: 0, conveyanceAllowance: 0, medicalAllowance: 0, specialAllowance: 0,
                        bonus: 0, overtimePay: 0, otherEarnings: 0, pf: 0, esi: 0, professionalTax: 0,
                        incomeTax: 0, loanRecovery: 0, otherDeductions: 0, bankAccount: '', upiId: ''
                        });
                  ...
                  const NumberInput = ({ label, value, onChange, required = false }) => (
  <div className="mb-4">
                ...
                <div className="bg-white w-full
                 ...
                 style={{ fontFamily: 'Open Sans',overflow:'auto'}}
                   >
                       ...
                       <table className="table w-full text-sm text-left text-gray-500 border-collapse min-w-[1220px]  " style={{ borderSpacing: '0 12px', borderCollapse: 'separate', }}>
                        <thead >
                            <tr className="sticky text-gray-500">
                                ...
                                <th className="" style={{ width: '90px' }}>DEPARTMENT</th>
                                <th className="" style={{ width: '90px' }}>DESIGNATION</th>
                                <th className="" style={{ width: '90px' }}>PHONE NUMBER</th>
                                <th className="" style={{ width: '100px' }}>PAYSLIP</th>
                                <th className="" style={{ width: '90px' }}>ACTION</th>
                            </tr>
                            </thead>
                            <tbody>
                           {employees
                         ...
                         {currentView === 'create-payslip' && (
                                ...
                                {/* Attachments Section */}
                                <div className="bg-white border-2 border-dashed border-gray-300 rounded-lg " style={{padding:'10px'}}>
                                    <h2 className="text-xl font-semibold mb-4 text-gray-600">Attachments</h2>
                                    <div className=" p-6 text-center">
                                    <Upload className="w-8 h-8 text-gray-400"  />
                                    <p className="text-gray-600">Upload supporting documents (PDF/Images)</p>
                                    <input type="file" multiple accept=".pdf,.jpg,.jpeg,.png" className="hidden" />
                                    <button type="button" className="mt-2 px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                                        Choose Files
                                    </button>
                                    </div>
                                </div>
                       ...
                       {/* Pagination */}
                   {currentView ==="employee-list" && (<div className="flex gap-1 justify-center">
